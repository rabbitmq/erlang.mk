...
[source,make]
CT_OPTS = -verbosity 50

The `CT_HOOKS` variable allows you to enable Common Test hooks:

[source,make]
TEST_DEPS = cowboy_ct_hook
CT_HOOKS = cth_surefire cowboy_ct_hook

Note you are still responsible for declaring relevant dependencies using e.g.
`TEST_DEPS`.
